# M3Mobile.M3Sdk.Xamarin

This is a Xamarin.Android SDK for M3Mobile devices, providing various functionalities and tools for application development.

NuGet package: [M3Mobile.M3Sdk.Xamarin 2.3.0-alpha01](https://www.nuget.org/packages/M3Mobile.M3Sdk.Xamarin/2.3.0-alpha01)

## Requirements

*   **MonoAndroid**: 7.0 or later
*   **Visual Studio**: 2017 or later

## Documentation

*   [M3SDK Xamarin Manual (English)](https://github.com/m3mobile/M3SDK/blob/develop/docs/M3SDK_Xamarin_Manual_en.md)
*   [M3SDK Xamarin Manual (Korean)](https://github.com/m3mobile/M3SDK/blob/develop/docs/M3SDK_Xamarin_Manual_kr.md)
